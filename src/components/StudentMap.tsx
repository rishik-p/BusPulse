...
const StudentMap = ({ busNumber, onEtaUpdate }: StudentMapProps) => {
    ...
    directionsService.route(
      ...
      (result, status) => {
        if (status === "OK" && result.routes.length > 0) {
          ...
          onEtaUpdate?.("Unable to calculate route");
        }
      }
    );  
  ...
  return (
    <div style={{ width: "100%", height: "100vh" }}>
      ...
      {isLoading && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-lg px-4 py-2 shadow-lg z-20">
